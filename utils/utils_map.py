    ...
    with open(RESULTS_FILES_PATH + "/results.txt", 'w') as results_file:
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            if len(prec)>0:
                ...
                # with open("Result.xlsx", 'a') as file:
                    # file.write(text.split('=')[0] + '\t' + text.split('=')[1] + '\t' + "{0:.2f}".format(F1[score_threhold_idx]) + '\t'  + "{0:.2f}%".format(rec[score_threhold_idx]*100) + '\t' + "{0:.2f}%".format(prec[score_threhold_idx]*100) + '\n')
                    # file.close()
            else:
